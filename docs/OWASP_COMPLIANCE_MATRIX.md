...
| Communications | Strong | Local-first, desktop CSP `connect-src 'self' https://api.pwnedpasswords.com`, HIBP range-only allowlist, no cloud service | Network review must be reopened if sync or update channels are added. |
